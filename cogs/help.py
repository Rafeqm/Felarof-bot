		...
		if commandName is not None:
			...
			if commandName2 is None:
				...
				if commandName2.usage is None:
					...
					embed.add_field(name=f"Usage", value=f"{self.client.command_prefix}{commandName2.name} {commandName2.usage}", inline=False)
				embed.add_field(name=f"Description", value=f"{commandName2.description}", inline=False)
				await ctx.channel.send(embed=embed)             
		else:
			embed = discord.Embed(title=f"Help page", description=f"{self.client.command_prefix}help (commandName), display the help list or the help data for a specific command.", color=randint(0, 0xffffff))
			embed.set_thumbnail(url=f'{self.client.user.avatar_url}')
			for i in self.client.commands:
